...
from backend.app.services.filter_options_service import get_filter_hierarchy, get_filter_options
...
@router.get("/filters/hierarchy")
def filters_hierarchy(db: Session = Depends(get_db), _auth: None = Depends(_require_viewer)) -> dict[str, Any]:
    return get_filter_hierarchy(db)
