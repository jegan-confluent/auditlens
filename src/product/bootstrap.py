...
def render_env_file(inputs: BootstrapInputs) -> str:
    ...
    lines = [
        ...
        "",
        "# Optional Schema Registry — emit both name variants because the",
        "# forwarder reads bare SCHEMA_REGISTRY_KEY/SECRET (src/forwarder/",
        "# config.py) while the backend reads SCHEMA_REGISTRY_API_KEY/SECRET",
        "# (backend/app/api/routes/settings.py).",
        f"SCHEMA_REGISTRY_URL={inputs.schema_registry_url if inputs.schema_registry_enabled else ''}",
        f"SCHEMA_REGISTRY_API_KEY={inputs.schema_registry_api_key if inputs.schema_registry_enabled else ''}",
        f"SCHEMA_REGISTRY_API_SECRET={inputs.schema_registry_api_secret if inputs.schema_registry_enabled else ''}",
        ...
        "# API_AUTH_ENABLED=false — set to true only if frontend has auth support.",
        "# The frontend has no token / session mechanism yet, so enabling Bearer",
        "# auth on the api breaks every dashboard view. The token-file generation",
        "# below still runs when the wizard's api_auth_enabled flag is true so",
        "# programmatic API consumers (curl, MCP, scripts) can flip this back to",
        "# true after wiring their own Authorization header.",
...
def render_secrets_env(inputs: BootstrapInputs) -> str:
    # SCHEMA_REGISTRY_KEY/SECRET previously lived here but were duplicated
    # in render_env_file; .env is the single source of truth so compose
    # interpolation works for environment-block lookups.
    # GF_SECURITY_ADMIN_PASSWORD previously lived here as a freshly-randomised
    # value that grafana's environment block overrode with
    # ${GRAFANA_ADMIN_PASSWORD} anyway — a wasted generation. The canonical
    # password is now GRAFANA_ADMIN_PASSWORD in .env.
    lines = [
