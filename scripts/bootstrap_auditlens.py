...
def collect_interactive_inputs(
    ...
    if 5 in completed:
        ...
        if inputs.persistence_enabled:
            # Postgres is the only supported backend now. The legacy
            # SQLite hot cache is gone — Postgres is reached via
            # DATABASE_URL through SQLAlchemy in every container.
            # PERSISTENCE_DB_PATH is still rendered into .env (the
            # SQLiteProductStore class still understands it for demo
            # mode when ENABLE_SQLITE_HOT_CACHE=true) but no longer
            # prompted for: the default tracks deployment_mode so
            # the path is sane if anything ever opts back in.
            inputs.persistence_backend = "postgres"
            print()
            print(cyan("  Persistence backend"))
            print(dim("    Using Postgres persistence (recommended)."))
            inputs.persistence_db_path = (
...
_DISABLED_NOTIFICATIONS_YAML = """# AuditLens Notification Configuration
# Auto-generated by ./setup because no webhook was provided during the
# wizard. Every destination here is enabled: false so a fresh install
# never produces notification-delivery errors on the placeholder URLs.
#
# To turn notifications on later:
#   1. Paste a real webhook URL into one of the entries below.
#   2. Flip its `enabled:` to true.
#   3. Save the file — the forwarder hot-reloads notifications.yml
...
def _maybe_write_disabled_notifications_yml(inputs: BootstrapInputs) -> None:
    """When the operator provided no Slack/webhook URL, drop a safe
    notifications.yml with every destination disabled.

    Two reasons this matters on a fresh install:
      1. docker-compose.prod.yml bind-mounts ./notifications.yml into
         api + forwarder. If the file is absent at compose-up time,
         Docker creates an empty *directory* at the mount source — the
         api process then fails to read its notifications config and
         logs a misleading "directory is not a file" warning.
      2. The legacy notifications.example.yml shipped with two
         destinations at enabled: true and placeholder webhook URLs. A
         literal copy as notifications.yml on an unconfigured stack
         produced an ERROR per action_required event. Defense-in-depth
         pairs with the notifier's silent placeholder-skip.

    The file is only written when:
      - inputs.slack_webhook is empty (operator skipped the prompt), AND
      - no notifications.yml exists on disk (don't clobber a
        previously-customised file from an earlier install).
    """
    if inputs.slack_webhook:
        return
    target = REPO_ROOT / "notifications.yml"
    if target.exists():
        return
    write_text_file(target, _DISABLED_NOTIFICATIONS_YAML, mode=0o644)
    info_line(
        "No webhook configured — wrote a disabled-by-default notifications.yml. "
        "Edit it to enable Slack/Teams/webhook destinations."
    )


def write_local_config(inputs: BootstrapInputs, token_json: str | None) -> list[Path]:
    """Write .env / .secrets / api token files to disk BEFORE compose up.

    Ordering invariant: called from main() at line ~2126, deploy_docker()
    runs at line ~2139. By the time compose starts containers, every file
    a container could bind-mount from ./secrets is already on disk:

      1. secrets/postgres_password.txt + grafana_admin_password.txt are
         (re)generated by _ensure_host_directories() during the Startup
         phase header block, also before deploy_docker.
      2. secrets/auditlens-api-tokens.json + auditlens-bootstrap-admin.token
         are written here.

    Resume edge case: if Phase 4 was already completed (api_auth_enabled
    True, mode "generate") and we're resuming, token_json may have been
    reconstructed by _reconstruct_token_json_on_resume. If the operator
    chose "existing" mode token_json is None — the on-disk file is the
    source of truth. We do NOT clobber an existing file in that case.

    File mode 0o444 (not 0o600): the api container's docker_entrypoint.py
    drops privileges to uid 1000 before reading the file. On Mac Docker
    Desktop the host uid does not map cleanly to 1000 inside the
    container, so 0o600 (owner-only) frequently produced "permission
    denied" + AuthConfig.from_env() raising + every authed route
    returning 503. Same posture as secrets/postgres_password.txt and
    secrets/grafana_admin_password.txt (both 0o444 for the same reason
    — see _ensure_secret_file).
    """
    ...
    if token_json:
        write_text_file(token_file_path, token_json, mode=0o444)
    if inputs.generated_admin_token:
        write_text_file(admin_token_path, inputs.generated_admin_token + "\n", mode=0o444)
    # Defensive guarantee: if api auth is enabled and somehow the token
    # JSON file is still absent at this point (token_json was None on a
    # resume of an "existing" mode install where the file got moved /
    # deleted between runs), refuse to proceed to deploy_docker. The
    # alternative is silent failure when the api container starts and
    # AuthConfig.from_env() raises FileNotFoundError → 503 on every
    # request. Better to fail loudly here with a clear next step.
    _maybe_write_disabled_notifications_yml(inputs)
    if inputs.api_auth_enabled and not token_file_path.exists():
        raise BootstrapError(
            f"API auth is enabled but {token_file_path} is missing. "
            "Re-run ./setup without resuming, or copy a valid token JSON "
            "file to that path before continuing."
        )
    return backups
...
def _check_auth_configuration(inputs: BootstrapInputs) -> None:
    """End-to-end auth round-trip against /events?limit=1.

    Sends the bootstrap admin token in the Authorization header when
    api_auth_enabled is true and the token file exists, so the probe
    becomes a real auth verification (token loaded by the api,
    DATABASE_URL works, AuthConfig.from_env succeeded) instead of the
    weaker "we get 401 → auth must be on" check we did before.

    Dispatches on status code:

      200 + sent_token        → success. Silent.
      401 + sent_token        → api rejected a valid token. Either the
                                token file inside the container doesn't
                                match the one on the host, or
                                AuthConfig.from_env failed to parse it.
                                Mismatch — warn.
      200 + api_auth_enabled=True + no token sent → auth was supposed to
                                be on but the api let an unauth call
                                through. Mismatch — warn.
      401 + api_auth_enabled=False → auth was supposed to be off, but
                                the api rejected us. Mismatch — warn.
      401 + api_auth_enabled=True + no token sent → expected (token
                                file not on disk for some reason but
                                api still enforces auth). Silent.
      403                     → role mismatch. Warn.
      503                     → DB or AuthConfig.from_env failure.
                                Escalate to BootstrapError so the
                                centralized failure handler renders the
                                API diagnosis block. Never warns
                                in-band on success path.
      transport_error         → api container crashed post-startup.
                                Escalate the same way.
    """
    from urllib.error import HTTPError, URLError
    from urllib.request import Request, urlopen

    # Load the bootstrap admin token if present. The token file is
    # written by write_local_config() in the same run (mode 0o444),
    # and is the plain-string form of the matching record inside
    # secrets/auditlens-api-tokens.json that the api container reads.
    # Missing file → fall back to the un-authed probe (preserves the
    # legacy semantics for installs where api_token_mode is "existing"
    # and the operator hand-managed the token file).
    admin_token = ""
    admin_token_path = REPO_ROOT / "secrets" / "auditlens-bootstrap-admin.token"
    if inputs.api_auth_enabled and admin_token_path.exists():
        try:
            admin_token = admin_token_path.read_text(encoding="utf-8").strip()
        except OSError:
            admin_token = ""
    sent_token = bool(admin_token)
    request_headers = {"Authorization": f"Bearer {admin_token}"} if sent_token else {}

    def _probe_once() -> tuple[int | None, Exception | None]:
        try:
            req = Request(
                "http://localhost:8080/events?limit=1",
    ...
    # Short retry budget kept as defense-in-depth — pg_isready already
    # gates entry to this function, but the api's connection-pool
    # warm-up can lag a few seconds even after postgres accepts
    # connections. 30 s is now plenty.
    ...
    if status == 503:
        raise BootstrapError(_service_failure_message(
            ...
            "raised because API_AUTH_TOKEN_FILE is missing or "
            "unreadable inside the container "
            "(check ./secrets:/run/secrets mount is present). "
            "Auto-fix: make repair.",
        ))

    mismatch = False
    if sent_token:
        # We sent a valid-from-the-host token. 200 = round-trip works.
        # 401 = api rejected it → file mismatch / AuthConfig error.
        # 403 = role mismatch (bootstrap token should be admin → wide
        #       privilege; unexpected).
        if status == 401:
            mismatch = True
        elif status == 403:
            mismatch = True
        # status == 200 → success, silent return below.
    else:
        # Legacy un-authed probe semantics retained for installs that
        # came up without a bootstrap admin token file on disk.
    ...
    if sent_token:
        print(dim("      Probe sent Authorization: Bearer <bootstrap admin token>."))
        print(dim("      The api rejected it — likely the file inside the container"))
        print(dim("      does not match secrets/auditlens-api-tokens.json on the host."))
        print(dim("      Check ./secrets:/run/secrets:ro is mounted on the api service."))
